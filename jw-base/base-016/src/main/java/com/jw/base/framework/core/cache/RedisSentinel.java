...
public abstract class RedisSentinel {
	 ...
	 * 创建多个集群共享的连接池
	 */
	protected void createJedisPool(List<String> clusterName) {
		List<String> redisInfo = jedisSentinel.sentinelGetMasterAddrByName(clusterName.get(0));
	 ...
	 */
	protected void checkRedisSentinelServer(final RedisSentinel redisSentinel, final int againCheckTime, final List<String> clusterName) {
		...
		Runnable checkRedisSentinelRunnable = new Runnable() {
			...
			public void run() {
				try {
					while (true) {
						jedisSentinel.ping();
						List<String> masters = jedisSentinel.sentinelGetMasterAddrByName(clusterName.get(0));
