...
public class RedisLock {
	...
	private JwCache redisCache = (JwCache) AppUtil.getBean("redisImpl");
	...
	public boolean lock(long timeout, int expire) {
		...
		try {
			...
			while ((System.nanoTime() - nanoTime) < timeout) {
				...
				if (redisCache.setnx(this.key, LOCKED) == 1) {
					redisCache.expired(key, expire);// 设置锁过期时间是为了在没有释放锁的情况下锁过期后消失，不会造成永久阻塞
					this.lock = true;
					return this.lock;
				}
