 ...
 * @package com.jw.base.framework.core.tablecopy
 */
public class Table_LOG_GUESTINFOSEND_INFO{
	private static final String TABLE = "log_guestinfosend_info";
	private static final String ORI_TABLE = "log_guestinfosend_info";
	...
	static{
		...
		CELLS.add(new TableCopyDto("certificatetyp",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("crtf_country",StandardBasicTypes.STRING));
		...
		CELLS.add(new TableCopyDto("visatyp",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("visa_country",StandardBasicTypes.STRING));
		...
		CELLS.add(new TableCopyDto("visaoffice",StandardBasicTypes.STRING));
		...
		CELLS.add(new TableCopyDto("country",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("province",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("city_id",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("district",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("birthday",StandardBasicTypes.DATE));
		
		CELLS.add(new TableCopyDto("gender",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("nation",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("address",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("certificatepic_id",StandardBasicTypes.INTEGER));
		...
		CELLS.add(new TableCopyDto("scan_str1",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("scan_str2",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("scan_str3",StandardBasicTypes.STRING));
		
		CELLS.add(new TableCopyDto("scan_str4",StandardBasicTypes.STRING));
...
/*		CELLS.add(new TableCopyDto("business_uid",StandardBasicTypes.STRING));
	...
	public final static String getInsertSql(String tbl){
		StringBuilder sb = new StringBuilder();
		if(CollectionUtils.isNotEmpty(CELLS)){
			CELLS.forEach(m -> {
				if(sb.length() != 0){
			...
			sb.append(" ) ");
			sb.insert(0, " INSERT INTO " + TABLE + "_" + tbl + " ( ");
		}
		return sb.toString();
	}
	...
	*/
	public final static Integer save(TableToSave<String, Map<String, Map<Object, Type>>, Integer> trans,
			List<Map<String, Object>> data, Integer maxLen, Map<String, TableToDo<?>> param, String tblNm) throws Exception {
		Integer cnt = 0;
		//不存在数据直接返回
		if (CollectionUtils.isEmpty(data)) {
			return cnt;
		...
		//拼接insert 语句
		String insertSql = getInsertSql(tblNm);
		//拼接保存语句
		StringBuilder sql = new StringBuilder();
		Map<String, Map<Object, Type>> outMap = new HashMap<>();
		for (Map<String, Object> map : data) {
