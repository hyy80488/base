...
日期|作者|类型|内容
:----|:-----|:-----|:-----
2016/11/26|liubowen|新增|DateUtil.java新增
 */
public class DateUtil  {
	...
	/** 格式字符串：yyyy-MM */
	private static final String DATE_YYYYMM_FORMAT= "yyyy-MM";
	...
	*/
	 ...
	 * @throws ParseException 
	 */
	public static Date getMonthAfter(Date sourse, int months) throws ParseException {
		Calendar now = Calendar.getInstance();
		now.setTime(sourse);
		now.set(Calendar.MONTH, now.get(Calendar.MONTH) + months);

		SimpleDateFormat sd = new SimpleDateFormat("yyyy-MM-dd");
		String[] ACCEPT_DATE_FORMATS = { "yyyy-MM-dd HH:mm:ss", "yyyy-MM-dd" };
...
//		}
	...
	*/
	 ...
	 * @return
	*/
	public static Date getLeastDate(Date... dates){
	...
	*/
	...
	public static final Time toTime(String time) {
		time += ":00:00";
		String[] strs = StringUtils.split(time, ":");
		strs = ArrayUtils.subarray(strs, 0, 3);
		Integer hh = Integer.valueOf(strs[0]);
		if(!NumberUtil.isExtent(0, 23, hh)){
			return null;
		}
		Integer mi = Integer.valueOf(strs[1]);
		if(!NumberUtil.isExtent(0, 59, mi)){
			return null;
		}
		Time t = Time.valueOf(String.join(":", strs));
		return t;
	}
}
