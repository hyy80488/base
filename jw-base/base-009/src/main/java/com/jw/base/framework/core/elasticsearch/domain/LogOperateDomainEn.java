...
@Document(indexName="log_operate_en", type="log_operate_en", shards=1, replicas=1 )
public class LogOperateDomainEn {
	@Id
	...
	//pattern 分词器：一个pattern类型的analyzer可以通过正则表达式将文本分成"terms"(经过token Filter 后得到的东西 )。
	//language 分词器：一个用于解析特殊语言文本的analyzer集合。
