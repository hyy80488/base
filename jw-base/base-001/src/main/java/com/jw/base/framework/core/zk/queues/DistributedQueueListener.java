 ...
 * 分布式队列处理类
 */
public class DistributedQueueListener implements IZKListener {
	...
	public void executor(CuratorFramework client) {
		...
		// 由于它们的地址是相同的，都是*/jw_queues/queues*，所以Curator会根据它们的空闲状态来分配新的任务，上面通过线程暂停5秒来拉开它们的处理间隔。
		queue1 = QueueBuilder.builder(client, consumer, serializer, "/jw_queues/queues").buildQueue();
		queue2 = QueueBuilder.builder(client, consumer, serializer, "/jw_queues/queues").buildQueue();
