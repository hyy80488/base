...
public class ZkLockTest {
	...
	public static void main(String[] args) {
		...
		zkClient.start();
		//testSharedLock(zkClient);
		//testReadWriterLock(zkClient);
		for (int i = 0; i < 4; i++) {
			testSharedSemaphore(zkClient, i);
		}
	}
	
	@Test
	public void testSharedLock_liubowen() throws Exception {
		// 1000 是重试间隔时间基数，3 是重试次数
		RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);
	...
	@Test
	public void testReadWriterLock_liubowen() throws Exception {
		// 1000 是重试间隔时间基数，3 是重试次数
		RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);
		CuratorFramework zkClient = createWithOptions("localhost:2181", retryPolicy, 2000, 10000);
		zkClient.start();
	...
	@Test
	public void testSharedSemaphore_liubowen() throws Exception {
		// 1000 是重试间隔时间基数，3 是重试次数
		RetryPolicy retryPolicy = new ExponentialBackoffRetry(1000, 3);
		CuratorFramework zkClient = createWithOptions("localhost:2181", retryPolicy, 2000, 10000);
		zkClient.start();
	 ...
	 * 共享锁测试
	 */
	private static void testSharedLock(CuratorFramework client) {
		String path = "/jw_lock/sharedlock";
		try {
	 ...
	 * @描述：测试读写锁
	 */
	public static void testReadWriterLock(CuratorFramework client) {
		String path = "/jw_lock/readwriter";
		try {
			if (client.checkExists().forPath(path) == null) {
				client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath(path, "".getBytes());
			}
	 ...
	 * 共享信号量测试
	 */
	private static void testSharedSemaphore(CuratorFramework client, final int x) {
		String path = "/jw_lock/sharedsemaphore";
		String path2 = "/jw_lock/semaphore";
		...
		try {
			...
			if (client.checkExists().forPath(path2) == null) {
				client.create().creatingParentsIfNeeded().withMode(CreateMode.PERSISTENT).forPath(path2, "".getBytes());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		final InterProcessSemaphoreV2 semaphoreV2 = new InterProcessSemaphoreV2(client, path, new SharedCount(client, path2, 2));
		List<Thread> jobs = new ArrayList<Thread>();
		for (int i = 0; i < 2; i++) {
	 ...
	 * @throws Exception
	 */
	public static void sharedLock(CuratorFramework client, String path) {
		// 这两个都是共享锁
	 ...
	 * @exception
	 */
	public static void readWriterLock(InterProcessLock lock) {
...
//			CuratorFramework clientinit = CuratorFrameworkFactory.builder().aclProvider(aclProvider)
//					.authorization(scheme, auth).connectionTimeoutMs(connectionTimeoutMs).connectString(connectString)
